...
public class TaskNotificationProducer {
    ...
    private static final Logger log = LoggerFactory.getLogger(TaskNotificationProducer.class);
    ...
    public Mono<Task> sendNotification(Task task) {
        return Mono.just(task)
                ...
                .map(it -> task)
                .doOnNext(it -> log.info("Task notification send successfully. - ({})", LocalDateTime.now()));
    }
}
